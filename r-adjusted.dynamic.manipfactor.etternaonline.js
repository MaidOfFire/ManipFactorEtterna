// ==UserScript==
// @name         rank-Adjusted Dynamic ManipFactor for EtternaOnline
// @namespace    http://tampermonkey.net/
// @version      0.3
// @description  Estimates the amount of manip from the replay data with adjustable weight function.
// @author
...
(function() {
    ...
    // Function to calculate deviations between keys with adjustable weight function
    function calculateDeviations(keyAData, keyBData) {
        ...
        // Now compute weights based on unimportances
        const N = unimportances.length;
        const unimportanceIndices = Array.from({ length: N }, (_, i) => i);
        unimportanceIndices.sort((i, j) => unimportances[i] - unimportances[j]);

        const ranks = Array(N);
        for (let rank = 0; rank < N; rank++) {
            ranks[unimportanceIndices[rank]] = rank + 1;
        }
    ...
    const initializeManipScoreDisplay = () => {
        fetchReplayData(replayData => {
            ...
            if (gradeOverallWrapper) {
                ...
                manipLabel.innerText = "adjMF (rank)"; // Label for Manipulation Factor
