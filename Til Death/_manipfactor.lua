-- Version: Updated to match Code C logic for Manipulation Factor calculation
-- For Til Death
...
-- Function to find all possible key pairs
local function FindKeyPairs(keymode)
    local keyPairs = {}
    for i = 0, keymode - 1 do
        for j = i + 1, keymode - 1 do
            table.insert(keyPairs, {i, j})
        end
    end
    return keyPairs
end

-- Function to determine the hand (left/right/middle) based on track number
local function GetHand(track, keymode)
    if keymode % 2 == 0 then
        if track < keymode / 2 then
            return "left"
        else
            return "right"
        end
    else
        if track < math.floor(keymode / 2) then
            return "left"
        elseif track > math.floor(keymode / 2) then
            return "right"
        else
            return "middle"
        end
...
local function GetManipFactor()
    ...
    -- Get key mode and specific key pairs (for 4K: 0-1 and 2-3)
    local keyPairs = {}
    local keymode = GetMaxTrack()
    if keymode == 4 then
        keyPairs = {
            {0, 1}, -- Left hand
            {1, 0}, -- Left hand reverse
            {2, 3}, -- Right hand
            {3, 2}  -- Right hand reverse
        }
    else
        keyPairs = FindKeyPairs(keymode)
    end
...
-- Manip factor display (same as before)...
