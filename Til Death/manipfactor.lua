...
if aspectRatio < 1.6 then
    mfDisplayX = SCREEN_CENTER_X
    mfDisplayY = SCREEN_CENTER_Y
else
...
function FilterTable(low,high,x,eps)
    local y = {}
    for i = 1, #x do
...
-- Generate key data
function GenerateKeyData(offsetVector, timingVector, trackVector, trackNum, tntypeVector)
    local keyData = {}
...
t[#t + 1] = Def.ActorFrame {
    UIElements.TextToolTip(1, 1, "Common Large") .. {
        InitCommand = function(self)
            self:xy(mfDisplayX, mfDisplayY):halign(1):zoom(0.25)
            self:settext("MF:")
        end,
    ...
    UIElements.TextToolTip(1, 1, "Common Large") .. {
        Name = "ManipFactor",
        InitCommand = function(self)
            self:xy(mfDisplayX + 3, mfDisplayY):halign(0):zoom(0.25)
            self:maxwidth(350)
